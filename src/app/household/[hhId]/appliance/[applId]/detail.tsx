      ...
      <CardHeader>
        <CardTitle className="flex flex-row items-center">
          ...
          <Badge variant="outline" className="ml-2">
            {toTitleCase(insertSpaces(appl === undefined ? '-' : ApplianceType[appl.applianceType]) ?? '-')}
          </Badge>
          <Badge variant="outline" className="ml-2">
            {toTitleCase(insertSpaces(appl === undefined ? '-' : AppliancePriority[appl.priority]) ?? '-')}
          </Badge>
          <div className="flex-1" />
          <Badge variant={appl?.status === ApplianceStatus.On ? 'default' : 'outline'}>
            {toTitleCase(insertSpaces(appl === undefined ? '-' : ApplianceStatus[appl.status]) ?? '-')}
          </Badge>
        </CardTitle>
      </CardHeader>
    </Card>
