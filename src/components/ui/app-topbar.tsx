...
export function AppTopbar() {
  ...
  usePathname().split('/').forEach((value, index) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      ApiService.getSimCfg().then((res) => {
        setTime(new Date(res.data.simulationTime).toLocaleString("en-US", {
          year: "numeric",
          month: "2-digit",
          day: "2-digit",
          hour: "2-digit",
  ...
  return (
    <div className="sticky top-0 backdrop-blur realtive h-16 border-b items-center flex gap-2 px-4 z-10" >
      ...
      <Button variant='ghost' size='icon' className='w-7 h-7' onClick={() => setShowWeather(!showWeather)} >
        <Thermometer />
      </Button>
      {showWeather ? <Label>{temp}</Label> : null}

      <Button variant='ghost' size='icon' className='w-7 h-7' onClick={() => setShowTime(!showTime)} >
        <Clock />
      </Button>
